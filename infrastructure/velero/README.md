...
| [velero.hr.yaml](velero.hr.yaml) | HelmRelease (controller, node agent, BSL, AWS plugin, and `Schedule` CRs via chart `values.schedules`) |
...
Defined under `values.schedules` in the HelmRelease. The chart names resources `{release}-{key}` (release `velero` → `velero-hourly-backup`, `velero-daily-backup`, `velero-weekly-backup`).
